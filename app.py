...
def standup_call(channel_name, message):
    ...
    if ("ok" in result):
        ...
        channel.timestamp = result.channel.ts
        DB.session.commit()
    else:
...
def set_email_job(channel):
    ...
    if (channel.email):
        ...
        SCHEDULER.add_job(get_timestamp_and_send_email, 'cron', [channel.channel_name, channel.email], day_of_week='mon-fri', hour=22, minute=13, id=channel.channel_name + "_sendemail")
