    ...
    if request.method == 'POST':
        ...
        if form.validate():
            ...
            if not db.session.query(Channel).filter(Channel.channel_name == submitted_channel_name).count():
                ...
                sched.add_job(lambda: standup_call(channel.channel_name), 'cron', day_of_week='mon-fri', hour=0, id=channel.channel_name)
            else:
                ...
                sched.reschedule_job(channel.channel_name, trigger='cron', day_of_week='mon-fri', hour=0)
        else:
