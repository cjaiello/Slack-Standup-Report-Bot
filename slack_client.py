...
def get_standup_replies_for_message(timestamp, channel_name):
  if SLACK_CLIENT:
    ...
    Logger.log(str({'channel': channel_id, 'latest': timestamp}), Logger.info)
    ...
    if (result["ok"]):
        Logger.log("Successfully got standup replies for message", Logger.info) # Issue 25: eventType: GetStandupReports
        # Only do the following if we actually got replies
        replies = result["messages"]
        Logger.log(str(replies), Logger.info) # Issue 25: eventType: GetStandupReports
        if (replies is not None):
            standup_results = []
            for standup_status in replies:
              Logger.log("Raw reply is: " + str(standup_status), Logger.info) # Issue 25: eventType: GetStandupReports
          ...
          Logger.log("We got back replies object but it was empty", Logger.info) # Issue 25: eventType: GetStandupReports
    else:
